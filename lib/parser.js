...

{
    ...
    internals.specials = function (code: number) {
...

{
    ...
    internals.c0Controls = function (code: number) {
...

{
    ...
    internals.c1Controls = function (code: number) {

        return lookup[code];
    };
}

internals.isSurrogate = function (code: number) {
...
internals.checkIpV6 = function (items: string[]) {
...
export type ParserOptions = {|
    normalizeUnnecessaryQuoted: boolean;
|};
...
export default class Parser {
    ...
    _normalizeUnnecessaryQuoted: boolean;

    /**
     * @param email The email address to parse.
     * @param diagnoses The Diagnoses object to which the parser will report
     *   problems with the email address.
     */
    constructor(email: string, diagnoses: Diagnoses, { normalizeUnnecessaryQuoted = false }: ParserOptions = {}) {
    ...
    /**
     */
    diagnose(type: number, index: number = this._reader.prevIndex) {
     ...
     *   obs-FWS =   1*([CRLF] WSP)
     *
     * @param initPrevRune The rune that prompted the FWS parse.
     */
    parseFWS(initPrevRune: string) {

        let prevRune = initPrevRune;
        let crlfCount = 0;
     ...
     *
     * @return The quoted rune.
     */
    parseQuotedPair({ allowMalformedUnicode = false }: { allowMalformedUnicode: boolean } = {}): string | null {
        ...
        else if (!allowMalformedUnicode && internals.isSurrogate(code)) {
    ...
    parseComment() {
        ...
        for (const rune of this._reader) {
            switch (rune) {
                ...
                case '\\':
                    // Quoted pair
                    this.parseQuotedPair({ allowMalformedUnicode: true });
                    break;
            ...
            switch (rune) {
                ...
                case '"':
                    ...
                    if (!element) {
                        ...
                        const isQuoted = quotedString[0] === '"';
                        localLength += Buffer.byteLength(quotedString, 'utf8') + (isQuoted ? 0 : 1) * 2;
