...
internals.checkSurrogatePair = function (token, nextToken) {

    const currentTokenCodePoint = token.codePointAt(0);
    const tokenPairCodePoint = (token + nextToken).codePointAt(0);

    return currentTokenCodePoint === tokenPairCodePoint ?
        false :
        token + nextToken;
};
...
exports.validate = internals.validate = function (email, options, callback) {
    ...
    for (let i = 0; i < emailLength; ++i) {
        const pairCheck = internals.checkSurrogatePair(email[i], email[i + 1]);
